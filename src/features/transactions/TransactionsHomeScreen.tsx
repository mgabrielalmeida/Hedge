...
export function TransactionsHomeScreen({ onAppearance, onNewExpense, onNoAccounts }: { onAppearance: () => void; onNewExpense: () => void; onNoAccounts: () => void }) {
  ...
  return <Screen><ScrollView contentContainerStyle={styles.content}><View><Text variant="heading">Início</Text><Text tone="muted" style={{ marginTop: tokens.spacing.sm }}>Despesas pontuais registradas para teste do fluxo.</Text></View>{error ? <Text tone="negative">{error}</Text> : null}<View style={styles.list}>{expenses.length === 0 ? <Card><Text tone="muted">Nenhuma despesa registrada ainda.</Text></Card> : expenses.map((expense) => <Card key={expense.id}><Text variant="title">{expense.name}</Text><Text tone="negative">{formatBrazilianCurrency(expense.amountCents)}</Text><Text tone="muted" variant="caption">{expense.transactionDate}</Text></Card>)}</View><Button label="Nova despesa" onPress={onNewExpense} /><Button label="Personalizar aparência" onPress={onAppearance} variant="secondary" /></ScrollView></Screen>;
}
